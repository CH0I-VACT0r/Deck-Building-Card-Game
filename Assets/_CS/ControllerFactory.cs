...
/// '데이터'를 기반으로 실제 '컨트롤러' 객체(Object)를 생성하는 정적 클래스
public static class ControllerFactory
{

    // [플레이어]
    /// GameSessionData에 저장된 영주 타입에 맞는 PlayerController를 생성
    /// <param name="battleManager">새 컨트롤러에게 전달할 BattleManager 참조</param>
    /// <param name="playerPanel">새 컨트롤러에게 전달할 UI 패널</param>
    /// <returns> 생성된 PlayerController (또는 그 자식 클래스)</returns>
    /// 
    public static PlayerController CreatePlayerController(BattleManager battleManager, VisualElement playerPanel)
    {
        // 1. 플레이어가 선택한 영주 타입을 읽음
        string lordType = GameSessionData.SelectedLordType;

        // 2. 'switch' 문을 사용해, 타입에 맞는 컨트롤러를 생성
        //    (프로토타입 단계에서는 영주의 기본 체력을 여기에 하드코딩한다.)
        switch (lordType)
        {
            case "SevereCold":
            ...
            // 3. 만약 '메모장'에 이상한 값이 적혀있다면, 가장 '기본'이 되는 부모 클래스를 생성하여 게임이 멈추지 않게 합니다.
            default:
    ...
    // [몬스터]
    /// GameSessionData에 저장된 몬스터 덱 ID에 맞는 MonsterController를 생성합니다.
    /// <param name="battleManager">새 컨트롤러에게 전달할 BattleManager 참조</param>
    /// <param name="monsterPanel">새 컨트롤러에게 전달할 UI 패널</param>
    /// <returns>생성된 MonsterController</returns>
